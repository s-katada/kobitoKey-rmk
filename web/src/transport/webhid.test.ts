...
import {
  ...
  getPreviouslyAuthorizedKobitokeyDevices,
  isWebHidSupported,
  requestKobitokeyDevice,
  WebHidTransport,
} from './webhid';
...
describe('requestKobitokeyDevice', () => {
  afterEach(clearFakeHid);

  it('throws TransportError when WebHID is not available', async () => {
    await expect(requestKobitokeyDevice()).rejects.toBeInstanceOf(TransportError);
  });

  it('returns the picked device', async () => {
    const dev = createMockDevice() as unknown as HIDDevice;
    installFakeHid([dev], dev);
    await expect(requestKobitokeyDevice()).resolves.toBe(dev);
  });

  it('returns null when the user cancels the picker', async () => {
    installFakeHid([], null);
    await expect(requestKobitokeyDevice()).resolves.toBeNull();
  });
});

describe('getPreviouslyAuthorizedKobitokeyDevices', () => {
  afterEach(clearFakeHid);

  it('filters out non-kobitokey devices', async () => {
    const kobitokey = createMockDevice() as unknown as HIDDevice;
    const notKobitokey = {
      ...createMockDevice(),
      vendorId: 0x1234,
      productId: 0x5678,
    } as unknown as HIDDevice;
    installFakeHid([kobitokey, notKobitokey], null);
    const got = await getPreviouslyAuthorizedKobitokeyDevices();
    expect(got).toEqual([kobitokey]);
  });

  it('returns an empty list when WebHID is not available', async () => {
    await expect(getPreviouslyAuthorizedKobitokeyDevices()).resolves.toEqual([]);
  });
});
...
describe('WebHidTransport', () => {
  ...
  it('round-trips a packet via sendAndReceive', async () => {
    ...
    // Simulate kobitokey echoing back after the send completes.
