/**
 * WebHID transport for the Vial wire protocol on kobitokey.
 *
 * Scope: ferry 32-byte packets in and out of kobitokey's vendor-defined
...
/**
 * Prompt the user to authorise a kobitokey device. Returns the picked
 * device, or null if the user cancelled the picker. Throws
 * `TransportError` when WebHID is unavailable.
 */
export async function requestKobitokeyDevice(): Promise<HIDDevice | null> {
  ...
  const devices = await navigator.hid.requestDevice({
    filters: [
      {
        vendorId: KOBITOKEY_VENDOR_ID,
        productId: KOBITOKEY_PRODUCT_ID,
...
/**
 * Return any kobitokey devices the user has previously authorised on this
 * origin. Useful for auto-reconnecting on app load without a click.
 */
export async function getPreviouslyAuthorizedKobitokeyDevices(): Promise<HIDDevice[]> {
  if (!isWebHidSupported()) return [];
  const devices = await navigator.hid.getDevices();
  return devices.filter(
    (d) =>
      d.vendorId === KOBITOKEY_VENDOR_ID &&
      d.productId === KOBITOKEY_PRODUCT_ID &&
 ...
 * a few milliseconds, so a slow reply means kobitokey is wedged or
...
export class WebHidTransport {
  ...
  static async open(
    ...
    if (!device.opened) {
      try {
        ...
        throw new TransportError('open-failed', `Failed to open kobitokey: ${String(err)}`);
      }
    }
   ...
   *
   * Vial doesn't have request ids: kobitokey echoes back on the same
  ...
  async sendAndReceive(packet: VialPacket): Promise<VialPacket> {
    ...
    const reply = new Promise<VialPacket>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new TransportError('receive-timeout', 'kobitokey did not reply within timeout'));
