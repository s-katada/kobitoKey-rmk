/**
 * Connection state machine for the kobitokey transport.
 *
 ...
 *                  talking to a kobitokey (UID matched)
 *   wrong-device - transport is open but the UID does not match a kobitokey
...
function deviceLabel(device: HIDDevice): string {
  ...
  return `kobitokey (${device.vendorId.toString(16)}:${device.productId.toString(16)})`;
}
...
export const useConnectionStore = create<ConnectionStore>((set, get) => {
  ...
  return {
    ...
    promptConnect: async () => {
      ...
      try {
        const device = await requestKobitokeyDevice();
        if (!device) {
          set({ state: { kind: 'idle' } });
          return;
        }
    ...
    trySilentReconnect: async () => {
      ...
      const previous = await getPreviouslyAuthorizedKobitokeyDevices();
