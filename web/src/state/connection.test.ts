...
import { KOBITOKEY_PRODUCT_ID, KOBITOKEY_VENDOR_ID, VIAL_USAGE, VIAL_USAGE_PAGE } from '../transport/types';
...
function createMockDevice(productName = 'kobitokey'): HIDDevice {
  return {
    opened: false,
...
const KOBITOKEY_UID = new Uint8Array([0xb9, 0xbc, 0x09, 0xb2, 0x9d, 0x37, 0x4c, 0xea]);
...
describe('useConnectionStore', () => {
  ...
  it('promptConnect transitions idle → connecting → ready on happy path', async () => {
    const device = createMockDevice('kobitokey');
    ...
    if (state.kind === 'ready') {
      expect(state.deviceName).toBe('kobitokey');
      expect(state.handshake.viaProtocolVersion).toBe(0x0009);
      expect(state.definitionFromCache).toBe(false);
    }
  });

  it('promptConnect goes to wrong-device when UID does not match kobitokey', async () => {
