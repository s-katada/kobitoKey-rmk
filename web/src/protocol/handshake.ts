 ...
 *   2. that we're talking to a kobitokey and not some other Vial board
 *      (`GetKeyboardId` cross-checked against `KOBITOKEY_KEYBOARD_UID`),
 ...
 * effectively how the web app discovers kobitokey's matrix size, layer
...
export const KOBITOKEY_KEYBOARD_UID: Uint8Array = new Uint8Array([
...
export interface HandshakeResult {
  ...
  /** True when the keyboard UID matches kobitokey's hardcoded value. */
  isKobitokey: boolean;
}

/** Byte-equality check used to confirm the connected device is kobitokey. */
...
export async function performHandshake(transport: WebHidTransport): Promise<HandshakeResult> {
  ...
  const isKobitokey = uidEquals(keyboardId.uid, KOBITOKEY_KEYBOARD_UID);
  ...
  if (totalSize === 0 || totalSize > 1024 * 64) {
    throw new TransportError(
      'send-failed',
      `Implausible keyboard definition size from kobitokey: ${totalSize} bytes`,
    );
  }
  ...
  return { viaProtocolVersion, keyboardId, definition, isKobitokey };
}
