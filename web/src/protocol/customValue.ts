/**
 * QMK Via "Custom Value" channel protocol for kobitokey-specific runtime
 * config.
 *
 ...
 * game.
 *
 * kobitokey claims channel `0xC0` and exposes one value per row of issue
...
/** kobitokey-editor channel. Must match `firmware/src/config.rs`. */
export const KOBITOKEY_CHANNEL = 0xc0;
...
/**
 * Static catalogue of every kobitokey setting that lives on the Custom
 * Value channel. Order matters only insofar as the UI walks the list;
 * the firmware looks each entry up by `id`.
 *
 * Keep this list in sync with:
 *   * `firmware/src/config.rs::KobitokeySettings`
...
/**
 * Via `CustomGetValue` for the kobitokey channel.
 *
 *   packet[0] = 0x08
 *   packet[1] = channel (0xC0)
 *   packet[2] = id
 */
export function buildKobitokeyGetValue(id: number): VialPacket {
  const p = emptyPacket();
  p[0] = ViaCommand.CustomGetValue;
  p[1] = KOBITOKEY_CHANNEL;
  p[2] = id & 0xff;
  return p;
}

/**
 * Via `CustomSetValue` for the kobitokey channel.
 *
 *   packet[0]    = 0x07
 *   packet[1]    = channel (0xC0)
 *   packet[2]    = id
 ...
 */
export function buildKobitokeySetValue(def: ValueDef, value: number): VialPacket {
  const p = emptyPacket();
...
/**
 * Via `CustomSave` for the kobitokey channel. RMK persists writes
 * immediately so this is a no-op, but we issue it for parity with
 * vial-gui-style tooling that expects an explicit flush.
 */
export function buildKobitokeyCustomSave(id: number): VialPacket {
  const p = emptyPacket();
 ...
 */
export function parseKobitokeyGetValue(def: ValueDef, reply: VialPacket): number {
  const bytes = valueBytes(def.type);
...
export async function getKobitokeyValue(transport: WebHidTransport, def: ValueDef): Promise<number> {
  const reply = await transport.sendAndReceive(buildKobitokeyGetValue(def.id));
  const value = parseKobitokeyGetValue(def, reply);
  if (value < def.min || value > def.max) return def.default;
  return value;
}

export async function setKobitokeyValue(
  transport: WebHidTransport,
  def: ValueDef,
  value: number,
): Promise<void> {
  const clamped = Math.max(def.min, Math.min(def.max, value | 0));
  await transport.sendAndReceive(buildKobitokeySetValue(def, clamped));
}

/**
 * Bulk read every kobitokey setting. Used on attach; the panel renders
 * the returned snapshot directly.
 */
export async function fetchKobitokeySettings(
  transport: WebHidTransport,
): Promise<Record<KobitokeySettingKey, number>> {
  const out = {} as Record<KobitokeySettingKey, number>;
  for (const def of KOBITOKEY_VALUES) {
    out[def.key] = await getKobitokeyValue(transport, def);
  }
  return out;
}
