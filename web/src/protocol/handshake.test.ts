...
import { KOBITOKEY_KEYBOARD_UID, performHandshake, uidEquals } from './handshake';

const KOBITOKEY_UID = new Uint8Array(KOBITOKEY_KEYBOARD_UID);
...
class FakeTransport {
  ...
  constructor(opts: FakeOptions = {}) {
    this.uid = opts.uid ?? KOBITOKEY_UID;
    this.defBytes = new TextEncoder().encode(
      opts.defText ??
        JSON.stringify({ name: 'kobitokey', matrix: { rows: 4, cols: 10 }, layouts: { keymap: [] } }),
    );
...
describe('uidEquals', () => {
  it('returns true for byte-identical arrays', () => {
    expect(uidEquals(KOBITOKEY_UID, KOBITOKEY_UID)).toBe(true);
    expect(uidEquals(new Uint8Array([1, 2]), new Uint8Array([1, 2]))).toBe(true);
  });

  it('returns false for different content', () => {
    expect(uidEquals(KOBITOKEY_UID, OTHER_UID)).toBe(false);
  });
...
describe('performHandshake', () => {
  ...
  it('completes the four-step dance against a kobitokey', async () => {
    const { t, raw } = fake();
    const result = await performHandshake(t);
    expect(result.viaProtocolVersion).toBe(0x0009);
    expect(result.isKobitokey).toBe(true);
    expect(result.definition.name).toBe('kobitokey');
  ...
  it('marks isKobitokey false when the UID does not match KOBITOKEY_KEYBOARD_UID', async () => {
    const { t } = fake({ uid: OTHER_UID });
    const result = await performHandshake(t);
    expect(result.isKobitokey).toBe(false);
  ...
  it('reassembles a multi-page definition', async () => {
    ...
    const result = await performHandshake(t);
    expect(result.definition.name).toBe('kobitokey-multipage');
