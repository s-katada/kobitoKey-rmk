/**
 * Read-only kobitokey battery status panel — surfaces central and peripheral
 * LiPo percentages from the patched RMK Vial Custom Channel 0xC0.
 *
 * macOS Bluetooth menu cannot display kobitokey's left and right batteries
 * separately (every dual-BAS experiment in the 2026-05-25 session caused
 * macOS to drop the entire battery display), so the kobitokey-config web UI
 ...
 *     `rmk::input_device::battery::KOBITOKEY_*_BATTERY_PERCENT` (atomics
 *     injected by `firmware/build.rs::patch_rmk_battery_kobitokey_atomics`).
 *   * `firmware/build.rs::patch_rmk_via_custom_get_kobitokey` rewrites the
...
import { getKobitokeyValue, KOBITOKEY_VALUES } from '../protocol/customValue';
import { useKobitokeySettingsStore } from '../state/kobitokeySettings';
import type { WebHidTransport } from '../transport/webhid';

const POLL_MS = 5_000;

const CENTRAL_DEF = KOBITOKEY_VALUES.find((v) => v.key === 'central_battery_percent');
const PERIPHERAL_DEF = KOBITOKEY_VALUES.find((v) => v.key === 'peripheral_battery_percent');
...
async function readBattery(transport: WebHidTransport): Promise<BatteryReading> {
  ...
  // request/reply round-trips — kobitokey's underlying RawHID endpoint isn't
  // pipelined.
  const central = CENTRAL_DEF ? await getKobitokeyValue(transport, CENTRAL_DEF) : null;
  const peripheral = PERIPHERAL_DEF ? await getKobitokeyValue(transport, PERIPHERAL_DEF) : null;
...
export function KobitokeyBatteryPanel() {
  const transport = useKobitokeySettingsStore(useShallow((s) => s.transport));
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4 p-4">
        <div className="flex flex-col items-center gap-1">
          ...
          <span
            data-testid="kobitokey-battery-central"
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <span
            data-testid="kobitokey-battery-peripheral"
