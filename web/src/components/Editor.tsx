 ...
 *   * `KobitokeySettingsPanel` — kobitokey-specific runtime knobs
 *
...
export function Editor() {
  ...
  const attachKobitokey = useKobitokeySettingsStore((s) => s.attach);
  const detachKobitokey = useKobitokeySettingsStore((s) => s.detach);
  ...
  useEffect(() => {
    if (connection.kind === 'ready') {
      ...
      void attachKobitokey(connection.transport);
    } else {
      detach();
      detachMacros();
      detachCombos();
  ...
  if (phase.kind === 'loading' || phase.kind === 'empty') {
    return (
      <p className="text-sm text-zinc-500 dark:text-zinc-400">kobitokey からキーマップを読み込み中…</p>
    );
  }
