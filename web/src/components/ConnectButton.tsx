...
function recoveryHint(errorKind: string): string {
  switch (errorKind) {
    ...
    case 'receive-timeout':
      return 'kobitokey から応答がありません。USB ケーブルを抜き差ししてから再試行してください。';
    case 'disconnected':
      return '通信中にケーブルが抜けました。差し直して再接続してください。';
    case 'send-failed':
      return 'kobitokey への書き込みに失敗しました。ケーブルを確認してから再試行してください。';
    default:
      return '';
  }
}
...
export function ConnectButton() {
  ...
  // Reattach to a previously-authorised kobitokey on first mount only.
  ...
  // must click「kobitokey に接続」(or refresh the page) to come back.
    ...
    return (
      <div className="space-y-3">
        <div className="rounded-md border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40 p-4 text-sm space-y-1">
          <p className="font-medium">これは kobitokey ではありません。</p>
          <p className="text-xs text-zinc-700 dark:text-zinc-300">
            キーボードが返した UID <code className="font-mono">{state.uidHex}</code> は kobitokey の
