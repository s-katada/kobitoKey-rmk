...
describe('ConnectButton', () => {
  ...
  it('renders "kobitokey に接続" button in idle state and calls promptConnect on click', async () => {
    setState({ kind: 'idle' });
    const { promptConnect } = stubStoreActions();
    render(<ConnectButton />);
    const button = screen.getByRole('button', { name: 'kobitokey に接続' });
  ...
  it('renders the ready panel with disconnect button', async () => {
    ...
    expect(
      screen.getByText(
        (_, node) =>
          node?.tagName === 'P' && (node?.textContent?.includes('kobitokey に接続しました') ?? false),
      ),
    ).toBeInTheDocument();
  ...
  it('renders the wrong-device warning and triggers disconnect on click', async () => {
    ...
    const { disconnect } = stubStoreActions();
    render(<ConnectButton />);
    expect(screen.getByText(/これは kobitokey ではありません/)).toBeInTheDocument();
