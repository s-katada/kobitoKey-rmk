...
fn patch_rmk_via_custom_get_kobu() {
    const MARKER: &str = "// kobu: via CustomGetValue handler for channel 0xC0 applied";
    const RMK_VERSION: &str = "0.8.2";
    ...
    let old_block = r#"            ViaCommand::CustomGetValue => {
    ...
    if !contents.contains(old_block) {
        panic!(
            "kobu: expected rmk-{RMK_VERSION} host/via/mod.rs CustomGetValue block missing in {}; \
             upstream may have changed — update firmware/build.rs",
            path.display()
        );
    }
